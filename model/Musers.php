...
class Musers {
    ...
    function edituser(){
        ...
        $itemid = $_GET["edit"];
        $userlist = $this->dbcon->query('SELECT * FROM users where id = '.$itemid. '');
        $phonelist = $this->dbcon->query('SELECT * FROM phone where uid = '.$itemid.'');
        ...
        //Создание формы
        $this->content="<h3>Редактирование пользователя</h3>";
        $frm = new Tform;
        ...
        if(isset($_POST["saveitem"])){
            ...
            if($data["phone1"] && $phone_con["phone1"] && $data["phone1"] != $phone_con["phone1"]){
                ...
                $sql =  'INSERT INTO phone (uid, phone, number) VALUES ('.$itemid.','.$data["phone1"].', 1)';
                $stmt = $this->dbcon->exec($sql);
            }
            ...
            if($data["phone2"] && $phone_con["phone2"]&& $data["phone2"] != $phone_con["phone2"]){
                ...
                $sql =  'INSERT INTO phone (uid, phone,number) VALUES ('.$itemid.','.$data["phone2"].', 2)';
                $stmt = $this->dbcon->exec($sql);
            }
            ...
            if($data["phone3"] && $phone_con["phone3"]&& $data["phone3"] != $phone_con["phone3"]){
                ...
                $sql =  'INSERT INTO phone (uid, phone, number) VALUES ('.$itemid.','.$data["phone3"].', 3)';
                $stmt = $this->dbcon->exec($sql);
            }
    ...
    function adduser(){
        ...
        if(isset($_POST["saveitem"])){
            ...
            if($data["phone1"]){
                ...
                $sql =  'INSERT INTO phone (uid, phone, number) VALUES ('.$user_last["id"].','.$data["phone1"].', 1)';
                $stmt = $this->dbcon->exec($sql);
            }
            if($data["phone2"]){
                $sql =  'INSERT INTO phone (uid, phone, number) VALUES ('.$user_last["id"].','.$data["phone2"].', 2)';
                $stmt = $this->dbcon->exec($sql);
            }
            if($data["phone3"]){
                $sql =  'INSERT INTO phone (uid, phone, number) VALUES ('.$user_last["id"].','.$data["phone3"].', 3)';
                $stmt = $this->dbcon->exec($sql);
            }
            header("Location: /");
        }   
        ...
        $frm->addparams("method=\"post\"");
        $frm->addparams("class=\"\"");
        $frm->addtextfield("first_name", "", "Введите имя пользователя", "Имя пользователя", 1);
        $frm->addtextfield("last_name", "", "Введите фамилию пользователя", "Фамилия пользователя", 1);
        $frm->addtextfield("email","", "Введите email пользователя", "Email пользователя", 1);
        $frm->addtextfield("company_name","", "Введите компанию пользователя", "Компания пользователя", 0);
        $frm->addtextfield("position","", "Введите должность пользователя", "Должность пользователя", 0);
        $frm->addtextfield("phone1","", "Введите первый телефон пользователя", "Первый телефон пользователя", 0);
        $frm->addtextfield("phone2","", "Введите второй телефон пользователя", "Второй телефон пользователя", 0);
        $frm->addtextfield("phone3","", "Введите третий телефон пользователя", "Третий телефон пользователя", 0);
        $frm->addsubmitfield("saveitem", "Сохранить");
        $this-> content.= 
            "<div class = \"container\">". 
            "<div class = \"row\">".
    ...
    function showlist(){
        ...
        $tbl->addcell("Телефон1");
        $tbl->addcell("Телефон2");
        $tbl->addcell("Телефон3");
        $tbl->finishrow();
        ...
        while ($user = $query->fetch()){
            ...
            $index = 0;
            foreach ($phone as $key => $phone_item){
                if($id == $phone_item['uid'] && $phone_item['number'] == 1){
            ...
            foreach ($phone as $key => $phone_item){
                if($id == $phone_item['uid'] && $phone_item['number'] == 2){
                    $tbl->addcell($phone_item['phone']);
                    $index++;
                }
            }
            ...
            foreach ($phone as $key => $phone_item){
                if($id == $phone_item['uid']  && $phone_item['number'] == 3){
                    $tbl->addcell($phone_item['phone']);
                    $index++;
                }
            }
